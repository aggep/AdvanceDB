...
from pyspark.sql.functions import to_timestamp, month, row_number
from pyspark.sql.functions import sin, cos, avg, sqrt, atan2, radians, asin
from pyspark.sql.functions import udf
import math


spark = SparkSession.builder.appName("Query 4 execution").getOrCreate()
...
# Perform a cross join to pair each crime with every station
cross_joined_df = df.crossJoin(lapd_stations_df)

# Calculate the distance for each crime-station pair
...
# Find the LAPD station with the smallest distance for each crime
min_distance_df = cross_joined_df.groupBy("DR_NO").agg(
    F.min("Distance_km").alias("Min_Distance")
)

# Join the original crime data with the station information for the closest station
result_df = cross_joined_df.join(
    min_distance_df,
    (cross_joined_df["DR_NO"] == min_distance_df["DR_NO"]) & 
    (cross_joined_df["Distance_km"] == min_distance_df["Min_Distance"]),
    "inner"
).select(df["*"],  "Distance_km", "PREC")

# Show the result
result_df.show(5)

# Group by Police Department (PREC) and calculate the required metrics
department_stats = result_df.groupBy("PREC").agg(
    count("DR_NO").alias("Number_of_Crimes"),
    avg("Distance_km").alias("Average_Distance_Km")
).orderBy(desc("Number_of_Crimes"))

# Display the results with only PREC, average distance, and crime count
department_stats.select("PREC", "Average_Distance_Km", "Number_of_Crimes").show()
spark.stop()

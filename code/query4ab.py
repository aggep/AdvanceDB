...
department_stats = result_df.groupBy("DIVISION").agg(
    count("DR_NO").alias("Crime_Count"),
    avg("Crime_Station_Distance").alias("Average_Distance_Km")
    ).orderBy(desc("Crime_Count"))

# Display the results with only PREC, average distance, and crime count
department_stats.select("DIVISION", "Average_Distance_Km", "Crime_Count").show()

...
# Group by year and calculate the count of crimes and the average distance
yearly_stats = result_df.groupBy("Year").agg(
    F.count("DR_NO").alias("Number_of_Crimes"),
    F.avg("Distance_km").alias("Average_Distance_Km")
).orderBy("Year")

# Display the results with only Year, Average Distance, and Number of Crimes
yearly_stats.select("Year", "Average_Distance_Km", "Number_of_Crimes").show()
